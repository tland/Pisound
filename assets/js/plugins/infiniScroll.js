...
(function() {
  InfiniScroll = function(view, collection, options) {
    options = options || { };
    ...
    self.onFetch = function() {
      self.lastCollectionSize = self.collection.length;
      self.options.onFetch();
    };

    self.fetchSuccess = function(collection, response) {
      if ((self.options.strict && collection.length >= (page + 1) * self.options.pageSize) || (!self.options.strict && response.h.length > 0)) {
        self.enableFetch();
        page += 1;
      } else {
        self.disableFetch();
      }
      self.options.success(self.view, collection, response);
    };
